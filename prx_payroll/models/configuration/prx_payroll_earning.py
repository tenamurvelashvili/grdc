...
class PRXPayrollEarning(models.Model):
    ...
    no_material_without_tax = fields.Boolean(string='არაფულადი - საპენსიოს გარეშე',tracking=True)
    ...
    @api.constrains('no_material', 'no_material_without_tax')
    def _check_material_flags(self):
        for rec in self:
            if rec.no_material and rec.no_material_without_tax:
                raise ValidationError("'არაფულადი სარგებელი' და 'არაფულადი - საპენსიოს გარეშე' ერთად მიუთითებელია")
