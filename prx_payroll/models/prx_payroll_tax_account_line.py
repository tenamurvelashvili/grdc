...
class PayrollTaxAccountLine(models.Model):
    _name = 'prx.payroll.tax.account.line'
    _description = 'Payroll Tax Account Line'
    _check_company_auto = True

    tax_id = fields.Many2one('prx.payroll.tax', string='Tax', required=True, ondelete='cascade')
    ...
    use_transaction_creditor = fields.Boolean(string='Use Transaction Creditor')
    tax_ids = fields.Many2many(
        'account.tax',
        'prx_payroll_tax_line_tax_rel',
        'line_id',
        'tax_id',
        string='Taxes',
        check_company=True,
        help='Attach tax code(s) to the generated move line without recalculating amounts.',
    )
    ...
    def prepare_move_line_vals(self, transaction):
        ...
        if self.use_transaction_creditor and transaction.creditor:
            partner = transaction.creditor

        amount = abs(transaction.amount) or 0.0
        ...
        return {
            ...
            'debit': debit,
            'credit': credit,
            # Mark as a base line for auto tax computation on journal entries.
